...
function timeSpanToString(startDate, endDate) {
  ...
  const mis = String(date.getMilliseconds()).padStart(3, '0');
  const seconds = String(date.getSeconds()).padStart(2, '0');
  const minutes = String(date.getMinutes()).padStart(2, '0');
  const hours = String(date.getUTCHours()).padStart(2, '0');
  return `${hours}:${minutes}:${seconds}.${mis}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const hourAngle = hour * 30 + date.getUTCMinutes() / 2;
  let minimumAngle = Math.abs(hourAngle - minuteAngle);
  if (minimumAngle > 180) {
    minimumAngle = 360 - minimumAngle;
  }
  return (minimumAngle * Math.PI) / 180;
}
